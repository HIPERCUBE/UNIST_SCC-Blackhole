...
int main(int argc, char **argv) {
    ...
    if (my_rank == 1) {
        BLACKHOLE_LAB_make_vision(collection, C_IMPORT_IMAGE_FILENAME, C_EXPORT_IMAGE_FILENAME, C_EXPORT_IMAGE_WIDTH,
                                  C_EXPORT_IMAGE_HEIGHT);
    }
...
void gather_results(RESULT *result, int number_of_results, RESULT *collection) {
    ...
    MPI_Aint member_displacement[4] = {offsetof(RESULT, status), offsetof(RESULT, x), offsetof(RESULT, y),
                                       offsetof(RESULT, pixel_index)};
